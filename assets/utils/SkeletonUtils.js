...
function retarget(target, source, options = {}) {
  ...
  options.preserveBoneMatrix =
    options.preserveBoneMatrix !== undefined
      ? options.preserveBoneMatrix
      : true;
  options.preserveBonePositions =
    options.preserveBonePositions !== undefined
      ? options.preserveBonePositions
      : true;
  options.useTargetMatrix =
    options.useTargetMatrix !== undefined ? options.useTargetMatrix : false;
  options.hip = options.hip !== undefined ? options.hip : "hip";
  options.hipInfluence =
    options.hipInfluence !== undefined
      ? options.hipInfluence
      : new Vector3(1, 1, 1);
...
function retargetClip(target, source, clip, options = {}) {
  ...
  options.fps =
    options.fps !== undefined
      ? options.fps
      : Math.max(...clip.tracks.map((track) => track.times.length)) /
        clip.duration;
  options.names = options.names || [];
  ...
  if (options.trim !== undefined) {
    start = Math.round(options.trim[0] * options.fps);
      ...
      Math.min(Math.round(options.trim[1] * options.fps), numFrames) - start;
