...
As perguntas formam uma fila por jogador. A primeira abre ao enviar o agente; as próximas aguardam sem consumir tempo e recebem **25 s completos quando chegam à sua vez**. A explicação da resposta permanece até **Próxima pergunta** ou até terminar sua exibição; expirar sem responder também avança a fila. A sequência ocupa o lugar do baralho e continua aberta mesmo depois que os agentes voltam. Uma pergunta isolada ainda se recolhe para a chamada compacta ao fim da tarefa. O botão **Baralho** permite recolher a janela sem descartar a fila; o prazo da pergunta já aberta continua correndo.
...
O desenho tem limite de 60 FPS e é suspenso quando a aba fica oculta; o servidor continua controlando os prazos da partida. O buffer WebGL só é redimensionado quando necessário, com o mesmo truncamento de pixels do Three.js. A câmera reutiliza o enquadramento, as medidas de texto usam um cache limitado a 512 entradas, e os esqueletos dos agentes são liberados ao sair de cena. Os testes de desempenho verificam essas operações e a liberação de recursos; não substituem medições de CPU, GPU e temperatura em aparelhos reais.
